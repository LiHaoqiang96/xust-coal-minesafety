...
var chart3
let  initChart = () => {
	chart1 = echarts.init(document.getElementById("qx1"),'dark');
	chart2 = echarts.init(document.getElementById("qx2"),'dark');
	chart4 = echarts.init(document.getElementById("qx4"),'dark');
	chart3 = echarts.init(document.getElementById("qx3"),'dark');
	var option1 = {
		//设置全局背景色
...
$(".but").click(function(){
    $('.shishishuju div.effect').hide()
    $('.shishishuju div.effect').slideDown(1500)
	//chart1.showLoading();
	...
	var refreshData = () => setInterval(function(){
		...
		$.ajax({
			...
			success:function(res){
				...
				times1.shift();    //时间数组（实际用来盛放X轴坐标值）
				nums1.shift();    //数据数组（实际用来盛放Y坐标值）
				times2.shift();
				nums2.shift();
				times3.shift();
				nums3.shift();
