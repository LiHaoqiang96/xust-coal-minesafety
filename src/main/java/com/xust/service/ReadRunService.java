...
//            jedis.close();
//        }
        List<Integer> notNullpres = new ArrayList<>();
        Jedis jedis = RedisPoll.getResource();
        for (int i = 0; i < pre.length; i++) {
            if(jedis.exists(pre[i]))
                notNullpres.add(i);
        ...
        jedis.close();
        CountDownLatch countDownLatch = new CountDownLatch(notNullpres.size());
        notNullpres.forEach(i -> ExecutorsUtil.cachedThreadPool.submit(
                    new SearchForSensor(countDownLatch,pre[i],concurrentHashMap)
        ));
